...
import (
	...
	////custom score cache package
	"k8s.io/kubernetes/pkg/scheduler/scorecache"
)
...
func (sched *Scheduler) schedulePod(ctx context.Context, fwk framework.Framework, state *framework.CycleState, pod *v1.Pod) (result ScheduleResult, err error) {
	...
	priorityList, err := sched.prioritizeNodesTop1WithRotatingCache(ctx, fwk, state, pod, feasibleNodes)
	if err != nil {
		return result, err
	}

	//host, _, err := selectHost(priorityList, numberOfHighestScoredNodesToReport)
	//// choose host safely even when priorityList is shortened by cache
	if len(priorityList) == 0 {
    	// schedulePod() returns (ScheduleResult, error), so return result + error
    	st := framework.NewStatus(framework.Unschedulable, "no nodes to schedule")
    	return result, st.AsError()
	}

	var host string
	if len(priorityList) == 1 {
    	host = priorityList[0].Name
	} else {
    	n := numberOfHighestScoredNodesToReport
    	if n > len(priorityList) {
        	n = len(priorityList)
    	}
    	var err error
    	host, _, err = selectHost(priorityList, n)
    	if err != nil {
        	return result, err
    	}
	}
...
////get highest scored node from cache
func (sched *Scheduler) prioritizeNodesTop1WithRotatingCache(
    	ctx context.Context,
    	fwk framework.Framework,
    	state *framework.CycleState,
    	pod *v1.Pod,
		...
		key := scorecache.BuildScoreKey(pod)
    	feasibleHash := scorecache.HashFeasibleNodes(feasibleNodes)

    	return sched.scoreCache.GetTop1AndPopOrCompute(ctx, key, feasibleHash, func() ([]framework.NodePluginScores, error) {
        	// original prioritizeNodes
        	return prioritizeNodes(ctx, sched.Extenders, fwk, state, pod, feasibleNodes)
    	})
}	